...
--*/
{
	...
	//一个扇区空间足够，写入全部数据
	if(BytesToWrite <= Vcb->Bpb.BytesPerSector - OffsetInSector)
	{
	...
	{
		ULONG RestToWrite = BytesToWrite;		//多余的字节数
		ULONG ToWrite =  Vcb->Bpb.BytesPerSector - OffsetInSector;	//在此扇区内需要写的字节数
		...
		Status = IopReadWriteSector( Vcb->DiskDevice,
									...
									ToWrite,
									FALSE);
		RestToWrite = RestToWrite - ToWrite;
		...
		while(RestToWrite > 0){
			...
			if(RestToWrite > Vcb->Bpb.BytesPerSector)
				ToWrite =  Vcb->Bpb.BytesPerSector;
			else
				ToWrite = RestToWrite;
			Status = IopReadWriteSector( Vcb->DiskDevice,		//	IopReadWriteSector不能跨扇区读写数据
										FirstSectorOfCluster,
										0,
										(PCHAR)Buffer,
										ToWrite,
	...
	// 如果文件长度增加了则必须修改文件的长度。
	if (Offset + BytesToWrite > File->FileSize || flag == 1) {
