	...
	)
{
	...
	for (;;){
		...
		if (0 == stricmp(Line, "ver")) {
			...
			ConsoleCmdUpThread(StdHandle, Arg);
			continue;
		} 
	...
	)
{
	...
	// 由进程 ID 获得进程控制块
	//
	Status = ObRefObjectById(ThreadID, PspThreadType, (PVOID*)&pThreadBlock);
	
	IntState = KeEnableInterrupts(FALSE);	// 关中断
	if(pThreadBlock->Priority > 0 && pThreadBlock->Priority < 8)
	{
		// 如果是处于运行状态或阻塞状态,直接设置优先级。
		if(pThreadBlock->State == Running || pThreadBlock->State == Waiting){
			pThreadBlock->Priority = 8;
		}
		// 如果是处于就绪状态的线程，需要先将该线程移出队列，
		// 然后设置该线程的优先级为8，恢复线程的初始时间片大小和剩余时间片大小
		else if(pThreadBlock->State == Ready){
	 		PspUnreadyThread(pThreadBlock);		                     //移出就绪队列并将状态修改为Zero
	 		pThreadBlock->Priority = 8;								// 设置优先级为8
	 		pThreadBlock->InitTickNum = TICKS_OF_TIME_SLICE;		// 设置初始化时间片大小
	 		pThreadBlock->RemainderTicks = TICKS_OF_TIME_SLICE;			// 设置剩余时间片大小
	 		PspReadyThread(pThreadBlock);
	 	}
